# Script para Sincronizar Dados do Hero para VPS
# Execute: .\sincronizar-hero-vps.ps1
...
Write-Host "Sincronizando dados do Hero para VPS..." -ForegroundColor Green
Write-Host ""
...
# 2. Verificar se heroes.json existe
if (-not (Test-Path "dados_exportados\heroes.json")) {
    Write-Host "ERRO: Arquivo heroes.json nao encontrado!" -ForegroundColor Red
    exit 1
}
...
if (Get-Command sshpass -ErrorAction SilentlyContinue) {
    $env:SSHPASS = $VPS_PASS
    sshpass -e scp "dados_exportados\heroes.json" "${VPS_USER}@${VPS_IP}:${VPS_PATH}/dados_exportados/"
} else {
    scp "dados_exportados\heroes.json" "${VPS_USER}@${VPS_IP}:${VPS_PATH}/dados_exportados/"
}
...
Write-Host "   OK: Dados importados e aplicacao reiniciada" -ForegroundColor Green
Write-Host ""

Write-Host "Sincronizacao concluida com sucesso!" -ForegroundColor Green
Write-Host ""
Write-Host "Verifique no site: https://central-rnc.com.br" -ForegroundColor Cyan
Write-Host ""
